...
async def account(update: Update, context: CallbackContext) -> None:
    ...
    if user and user.get('status') == 'active' and not context.user_data.get('logged_out', False):
        # Якщо користувач авторизований і не вийшов з акаунту, показуємо інформацію про акаунт
        keyboard = [
        ...
        # Якщо користувач не авторизований або вийшов з акаунту, показуємо кнопки для створення акаунту, входу або відміни
        keyboard = [
...
async def button_callback(update: Update, context: CallbackContext) -> None:
    ...
    try:
        if query.data == 'create_account_yes':
            ...
            context.user_data['awaiting_login_for_login'] = True  # Позначаємо, що очікуємо логін для входу
        elif query.data == 'edit_account':
            await query.edit_message_text(text="Введіть ваш пароль для підтвердження:")
            context.user_data['awaiting_password_for_edit'] = True  # Позначаємо, що очікуємо пароль для редагування
        elif query.data == 'logout':
            # Позначаємо, що користувач вийшов з акаунту
            context.user_data['logged_out'] = True
            # Оновлюємо статус акаунта на 'pasive'
            users_collection.update_one({"user_id": query.from_user.id}, {"$set": {"status": "pasive"}})
            # Очищаємо всі дані, пов'язані з попереднім акаунтом
            context.user_data.clear()
            # Показуємо початкове меню з кнопками "Створити новий акаунт", "Вхід" та "Відмінити"
            keyboard = [
                [InlineKeyboardButton("Створити новий акаунт", callback_data='create_account_yes')],
                [InlineKeyboardButton("Вхід", callback_data='login')],
                [InlineKeyboardButton("Відмінити", callback_data='cancel')]
    ...
    if 'awaiting_login' in context.user_data:
        ...
        if user_code == context.user_data['verification_code']:
            ...
            users_collection.insert_one({
                ...
                "status": "active",  # Статус акаунта - активний
                "basket": 0,
                "all_orders": 0
            })
        ...
        # Користувач ввів логін для входу
        login = update.message.text
        ...
        if user:
            if user.get('status') == 'pasive':
                ...
                context.user_data['awaiting_password_for_unlock'] = True  # Позначаємо, що очікуємо пароль для розблокування
                context.user_data['login_for_unlock'] = login  # Зберігаємо логін для розблокування
                del context.user_data['awaiting_login_for_login']  # Видаляємо прапор очікування логіну
                return
            context.user_data['login'] = login
            # Генеруємо код підтвердження для входу
            verification_code = generate_verification_code()
            context.user_data['verification_code'] = verification_code
            ...
            if send_verification_email(user['email'], verification_code):
                ...
                context.user_data['awaiting_verification_for_login'] = True  # Позначаємо, що очікуємо код підтвердження
                del context.user_data['awaiting_login_for_login']  # Видаляємо прапор очікування логіну
            else:
                await update.message.reply_text("Помилка при надсиланні коду підтвердження. Спробуйте ще раз.")
        else:
        ...
        # Користувач ввів пароль для редагування
        password = update.message.text
        user = users_collection.find_one({"user_id": user_id, "password": password})
        if user:
            await update.message.reply_text("Введіть новий логін:")
            context.user_data['awaiting_new_login'] = True  # Позначаємо, що очікуємо новий логін
            del context.user_data['awaiting_password_for_edit']  # Видаляємо прапор очікування паролю
        else:
            await update.message.reply_text("Невірний пароль. Спробуйте ще раз.")
    elif 'awaiting_new_login' in context.user_data:
        # Користувач ввів новий логін
